...
mem_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
    // the third parameter uses a trick(0-KERNBASE)
...
page_free(struct PageInfo *pp)
{
    ...
    // 上面注释提到pp_link不为NULL是指一个本来就在free list中的页
    // page_alloc会将分配的page的pp_link设为NULL
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
    if(va < UTOP) {
        panic("boot_map_region: va is lower than UTOP\n");
        return;
    }
    size_t i;
    pte_t *ptep;
